...
import com.avicsafety.NewShenYangTowerComService.PowerManager.push.service.PositionService;
import com.avicsafety.NewShenYangTowerComService.R;
import com.avicsafety.NewShenYangTowerComService.model.M_LoginInfo;
import com.avicsafety.NewShenYangTowerComService.service.LoginManager;
...
//				  loginManager.loginDD(oThis,username,userPwd);
	...
	public void login(final Context context, final String username, final String password, final String imei, final OnNetworkAccessToModelListener listener) {
		//这个处理网络和数据的方法 应该放到 Service 层

		RequestParams params = new RequestParams(com.avicsafety.NewShenYangTowerComService.conf.Constants.LOGIN);
			...
			public void onSuccess(String result) {
				...
				try {
					...
					if (code.equals("200")) {
						...
						com.avicsafety.NewShenYangTowerComService.conf.Constants.setUserInfo(context, datas);
						com.avicsafety.NewShenYangTowerComService.conf.Constants.setSubmitUser(username, context);
	...
	public void goToMain() {
		//开启了一些后台服务 信鸽 位置 这些东西最好不要放到即将销毁的页面中
		startServer();
		//用户名
