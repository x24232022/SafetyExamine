package com.avicsafety.NewShenYangTowerComService.model;

public class Mgdxq {
	private String resprocess;
	private String validateContent;
	private String workContent;
	private String belongsuburb;
	private String no;
	private String specialist;
	private String teleAlarmreaanalysis;
	private String auditPerson;
	private String lineMaintenanceName;
	private String customer;
	private String troubRecoverCondition;
	private String dutyLocation;
	private String belongbts;
	private String alarmtype;
	private String cleartype;
	private String isfind;
	private String operation_bak3;
	private String orientation;
	private String date_bak2;
	private String alarmrealist;
	private String alarmreason;
	private String processLimit;
	private String nodeCn;
	private String applyhardware;
	private String alarmId;
	private String date_bak3;
	private String bak4;
	private String alarmtally;
	private String bak5;
	private String waitOpinion;
	private String feedbackinfo;
	private String bak9;
	private String cov_load;
	private String auditPersonTel;
	private String scopeextent;
	private String latitude;
	private String transferUserId;
	private String bak1;
	private String rep_dis;
	private String bak2;
	private String bak3;
	private String teleSubAlarmreaanalysis;
	private String waitAuditPersonTel;
	private String alarmlocation;
	private String waitAuditPerson;
	private String faulttitle;
	private String lineMaintenanceNameid;
	private String belongbsc;
	private String sla_resolutiontime;
	private String inserttime;
	private String waitRes;
	private String equipLevel;
	private String findTime;
	private String alarmtime;
	private String signInDate;
	private String isAdjustment;
	private String alarmStaId;
	private String orderId;
	private String alarmreaanalysis;
	private String appContent;
	private String engineertel;
	private String vendor;
	private String theme;
	private String urgePersonTel;
	private String oriAlarmId;
	private String alarmlasttime;
	private String appPersonTel;
	private String auditRes;
	private String waitAuditDate;
	private String serialNo;
	private String initiatorDepartment;
	private String cellulartype;
	private String auditDate;
	private String transferDate;
	private String acknowledged;
	private String assigntime;
	private String waitreason2;
	private String cov_area;
	private String hanprofessional;
	private String location;
	private String gzriselevel;
	private String wglineMaintenanceNametp;
	private String alarmlevel;
	private String networktype;
	private String urgeDate;
	private String engineerid;
	private String equipType;
	private String engineername;
	private String results;
	private String troubLevel;
	private String rejectContent;
	private String longitude;
	private String dutyBelong;
	private String createType;
	private String waitreason;
	private String faultautorectime;
	private String transferUserName;
	private String initiatorTel;
	private String alarmdevsite;
	private String status;
	private String sla_assignetime;
	private String urgeContent;
	private String belongs;
	private String alarmdevserno;
	private String wglineMaintenanceNameid;
	private String rejectreason;
	private String lineMaintenanceNametp;
	private String sla_closetime;
	private String isWait;
	private String btslevel;
	private String wglineMaintenanceName;
	private String transferUserTel;
	private String alarmsource;
	private String troubAddress;
	private String alarmHandleLevel;
	private String failuredescription;
	private String teleEquipLevel;
	private String alarmdevtype;
	private String id;
	private String alarmAutoHandle;
	private String isAnyQuestion;
	private String urgePerson;
	private String alarmProvince;
	private String networkLevel;
	private String maintainUnit;
	private String ackperson;
	private String alarmServEffect;
	private String appDate;
	private String auditOpinion;
	private String appPerson;
	private String engineeremail;
	private String origin;
	private String oppositeCreateDate;
	private String alarmavgrenewtime;
	private String com_stat;
	private String inmsalarmflag;
	private String alarmSubType;
	private String originTime;
	private String sendinstancename;
	private String initiator;
	private String rejectreason2;
	private String woId;
	private String sla_createtime;
	private String tyneid;
	private String applyrm;

	public String getAcknowledged() {
		return acknowledged;
	}

	public String getAckperson() {
		return ackperson;
	}

	public String getAlarmAutoHandle() {
		return alarmAutoHandle;
	}

	public String getAlarmavgrenewtime() {
		return alarmavgrenewtime;
	}

	public String getAlarmdevserno() {
		return alarmdevserno;
	}

	public String getAlarmdevsite() {
		return alarmdevsite;
	}

	public String getAlarmdevtype() {
		return alarmdevtype;
	}

	public String getAlarmHandleLevel() {
		return alarmHandleLevel;
	}

	public String getAlarmId() {
		return alarmId;
	}

	public String getAlarmlasttime() {
		return alarmlasttime;
	}

	public String getAlarmlevel() {
		return alarmlevel;
	}

	public String getAlarmlocation() {
		return alarmlocation;
	}

	public String getAlarmProvince() {
		return alarmProvince;
	}

	public String getAlarmreaanalysis() {
		return alarmreaanalysis;
	}

	public String getAlarmrealist() {
		return alarmrealist;
	}

	public String getAlarmreason() {
		return alarmreason;
	}

	public String getAlarmServEffect() {
		return alarmServEffect;
	}

	public String getAlarmsource() {
		return alarmsource;
	}

	public String getAlarmStaId() {
		return alarmStaId;
	}

	public String getAlarmSubType() {
		return alarmSubType;
	}

	public String getAlarmtally() {
		return alarmtally;
	}

	public String getAlarmtime() {
		return alarmtime;
	}

	public String getAlarmtype() {
		return alarmtype;
	}

	public String getAppContent() {
		return appContent;
	}

	public String getAppDate() {
		return appDate;
	}

	public String getApplyhardware() {
		return applyhardware;
	}

	public String getApplyrm() {
		return applyrm;
	}

	public String getAppPerson() {
		return appPerson;
	}

	public String getAppPersonTel() {
		return appPersonTel;
	}

	public String getAssigntime() {
		return assigntime;
	}

	public String getAuditDate() {
		return auditDate;
	}

	public String getAuditOpinion() {
		return auditOpinion;
	}

	public String getAuditPerson() {
		return auditPerson;
	}

	public String getAuditPersonTel() {
		return auditPersonTel;
	}

	public String getAuditRes() {
		return auditRes;
	}

	public String getBak1() {
		return bak1;
	}

	public String getBak2() {
		return bak2;
	}

	public String getBak3() {
		return bak3;
	}

	public String getBak4() {
		return bak4;
	}

	public String getBak5() {
		return bak5;
	}

	public String getBak9() {
		return bak9;
	}

	public String getBelongbsc() {
		return belongbsc;
	}

	public String getBelongbts() {
		return belongbts;
	}

	public String getBelongs() {
		return belongs;
	}

	public String getBelongsuburb() {
		return belongsuburb;
	}

	public String getBtslevel() {
		return btslevel;
	}

	public String getCellulartype() {
		return cellulartype;
	}

	public String getCleartype() {
		return cleartype;
	}

	public String getCom_stat() {
		return com_stat;
	}

	public String getCov_area() {
		return cov_area;
	}

	public String getCov_load() {
		return cov_load;
	}

	public String getCreateType() {
		return createType;
	}

	public String getCustomer() {
		return customer;
	}

	public String getDate_bak2() {
		return date_bak2;
	}

	public String getDate_bak3() {
		return date_bak3;
	}

	public String getDutyBelong() {
		return dutyBelong;
	}

	public String getDutyLocation() {
		return dutyLocation;
	}

	public String getEngineeremail() {
		return engineeremail;
	}

	public String getEngineerid() {
		return engineerid;
	}

	public String getEngineername() {
		return engineername;
	}

	public String getEngineertel() {
		return engineertel;
	}

	public String getEquipLevel() {
		return equipLevel;
	}

	public String getEquipType() {
		return equipType;
	}

	public String getFailuredescription() {
		return failuredescription;
	}

	public String getFaultautorectime() {
		return faultautorectime;
	}

	public String getFaulttitle() {
		return faulttitle;
	}

	public String getFeedbackinfo() {
		return feedbackinfo;
	}

	public String getFindTime() {
		return findTime;
	}

	public String getGzriselevel() {
		return gzriselevel;
	}

	public String getHanprofessional() {
		return hanprofessional;
	}

	public String getId() {
		return id;
	}

	public String getInitiator() {
		return initiator;
	}

	public String getInitiatorDepartment() {
		return initiatorDepartment;
	}

	public String getInitiatorTel() {
		return initiatorTel;
	}

	public String getInmsalarmflag() {
		return inmsalarmflag;
	}

	public String getInserttime() {
		return inserttime;
	}

	public String getIsAdjustment() {
		return isAdjustment;
	}

	public String getIsAnyQuestion() {
		return isAnyQuestion;
	}

	public String getIsfind() {
		return isfind;
	}

	public String getIsWait() {
		return isWait;
	}

	public String getLatitude() {
		return latitude;
	}

	public String getLineMaintenanceName() {
		return lineMaintenanceName;
	}

	public String getLineMaintenanceNameid() {
		return lineMaintenanceNameid;
	}

	public String getLineMaintenanceNametp() {
		return lineMaintenanceNametp;
	}

	public String getLocation() {
		return location;
	}

	public String getLongitude() {
		return longitude;
	}

	public String getMaintainUnit() {
		return maintainUnit;
	}

	public String getNetworkLevel() {
		return networkLevel;
	}

	public String getNetworktype() {
		return networktype;
	}

	public String getNo() {
		return no;
	}

	public String getNodeCn() {
		return nodeCn;
	}

	public String getOperation_bak3() {
		return operation_bak3;
	}

	public String getOppositeCreateDate() {
		return oppositeCreateDate;
	}

	public String getOrderId() {
		return orderId;
	}

	public String getOriAlarmId() {
		return oriAlarmId;
	}

	public String getOrientation() {
		return orientation;
	}

	public String getOrigin() {
		return origin;
	}

	public String getOriginTime() {
		return originTime;
	}

	public String getProcessLimit() {
		return processLimit;
	}

	public String getRejectContent() {
		return rejectContent;
	}

	public String getRejectreason() {
		return rejectreason;
	}

	public String getRejectreason2() {
		return rejectreason2;
	}

	public String getRep_dis() {
		return rep_dis;
	}

	public String getResprocess() {
		return resprocess;
	}

	public String getResults() {
		return results;
	}

	public String getScopeextent() {
		return scopeextent;
	}

	public String getSendinstancename() {
		return sendinstancename;
	}

	public String getSerialNo() {
		return serialNo;
	}

	public String getSignInDate() {
		return signInDate;
	}

	public String getSla_assignetime() {
		return sla_assignetime;
	}

	public String getSla_closetime() {
		return sla_closetime;
	}

	public String getSla_createtime() {
		return sla_createtime;
	}

	public String getSla_resolutiontime() {
		return sla_resolutiontime;
	}

	public String getSpecialist() {
		return specialist;
	}

	public String getStatus() {
		return status;
	}

	public String getTeleAlarmreaanalysis() {
		return teleAlarmreaanalysis;
	}

	public String getTeleEquipLevel() {
		return teleEquipLevel;
	}

	public String getTeleSubAlarmreaanalysis() {
		return teleSubAlarmreaanalysis;
	}

	public String getTheme() {
		return theme;
	}

	public String getTransferDate() {
		return transferDate;
	}

	public String getTransferUserId() {
		return transferUserId;
	}

	public String getTransferUserName() {
		return transferUserName;
	}

	public String getTransferUserTel() {
		return transferUserTel;
	}

	public String getTroubAddress() {
		return troubAddress;
	}

	public String getTroubLevel() {
		return troubLevel;
	}

	public String getTroubRecoverCondition() {
		return troubRecoverCondition;
	}

	public String getTyneid() {
		return tyneid;
	}

	public String getUrgeContent() {
		return urgeContent;
	}

	public String getUrgeDate() {
		return urgeDate;
	}

	public String getUrgePerson() {
		return urgePerson;
	}

	public String getUrgePersonTel() {
		return urgePersonTel;
	}

	public String getValidateContent() {
		return validateContent;
	}

	public String getVendor() {
		return vendor;
	}

	public String getWaitAuditDate() {
		return waitAuditDate;
	}

	public String getWaitAuditPerson() {
		return waitAuditPerson;
	}

	public String getWaitAuditPersonTel() {
		return waitAuditPersonTel;
	}

	public String getWaitOpinion() {
		return waitOpinion;
	}

	public String getWaitreason() {
		return waitreason;
	}

	public String getWaitreason2() {
		return waitreason2;
	}

	public String getWaitRes() {
		return waitRes;
	}

	public String getWglineMaintenanceName() {
		return wglineMaintenanceName;
	}

	public String getWglineMaintenanceNameid() {
		return wglineMaintenanceNameid;
	}

	public String getWglineMaintenanceNametp() {
		return wglineMaintenanceNametp;
	}

	public String getWoId() {
		return woId;
	}

	public String getWorkContent() {
		return workContent;
	}

	public void setAcknowledged(String acknowledged) {
		this.acknowledged = acknowledged;
	}

	public void setAckperson(String ackperson) {
		this.ackperson = ackperson;
	}

	public void setAlarmAutoHandle(String alarmAutoHandle) {
		this.alarmAutoHandle = alarmAutoHandle;
	}

	public void setAlarmavgrenewtime(String alarmavgrenewtime) {
		this.alarmavgrenewtime = alarmavgrenewtime;
	}

	public void setAlarmdevserno(String alarmdevserno) {
		this.alarmdevserno = alarmdevserno;
	}

	public void setAlarmdevsite(String alarmdevsite) {
		this.alarmdevsite = alarmdevsite;
	}

	public void setAlarmdevtype(String alarmdevtype) {
		this.alarmdevtype = alarmdevtype;
	}

	public void setAlarmHandleLevel(String alarmHandleLevel) {
		this.alarmHandleLevel = alarmHandleLevel;
	}

	public void setAlarmId(String alarmId) {
		this.alarmId = alarmId;
	}

	public void setAlarmlasttime(String alarmlasttime) {
		this.alarmlasttime = alarmlasttime;
	}

	public void setAlarmlevel(String alarmlevel) {
		this.alarmlevel = alarmlevel;
	}

	public void setAlarmlocation(String alarmlocation) {
		this.alarmlocation = alarmlocation;
	}

	public void setAlarmProvince(String alarmProvince) {
		this.alarmProvince = alarmProvince;
	}

	public void setAlarmreaanalysis(String alarmreaanalysis) {
		this.alarmreaanalysis = alarmreaanalysis;
	}

	public void setAlarmrealist(String alarmrealist) {
		this.alarmrealist = alarmrealist;
	}

	public void setAlarmreason(String alarmreason) {
		this.alarmreason = alarmreason;
	}

	public void setAlarmServEffect(String alarmServEffect) {
		this.alarmServEffect = alarmServEffect;
	}

	public void setAlarmsource(String alarmsource) {
		this.alarmsource = alarmsource;
	}

	public void setAlarmStaId(String alarmStaId) {
		this.alarmStaId = alarmStaId;
	}

	public void setAlarmSubType(String alarmSubType) {
		this.alarmSubType = alarmSubType;
	}

	public void setAlarmtally(String alarmtally) {
		this.alarmtally = alarmtally;
	}

	public void setAlarmtime(String alarmtime) {
		this.alarmtime = alarmtime;
	}

	public void setAlarmtype(String alarmtype) {
		this.alarmtype = alarmtype;
	}

	public void setAppContent(String appContent) {
		this.appContent = appContent;
	}

	public void setAppDate(String appDate) {
		this.appDate = appDate;
	}

	public void setApplyhardware(String applyhardware) {
		this.applyhardware = applyhardware;
	}

	public void setApplyrm(String applyrm) {
		this.applyrm = applyrm;
	}

	public void setAppPerson(String appPerson) {
		this.appPerson = appPerson;
	}

	public void setAppPersonTel(String appPersonTel) {
		this.appPersonTel = appPersonTel;
	}

	public void setAssigntime(String assigntime) {
		this.assigntime = assigntime;
	}

	public void setAuditDate(String auditDate) {
		this.auditDate = auditDate;
	}

	public void setAuditOpinion(String auditOpinion) {
		this.auditOpinion = auditOpinion;
	}

	public void setAuditPerson(String auditPerson) {
		this.auditPerson = auditPerson;
	}

	public void setAuditPersonTel(String auditPersonTel) {
		this.auditPersonTel = auditPersonTel;
	}

	public void setAuditRes(String auditRes) {
		this.auditRes = auditRes;
	}

	public void setBak1(String bak1) {
		this.bak1 = bak1;
	}

	public void setBak2(String bak2) {
		this.bak2 = bak2;
	}

	public void setBak3(String bak3) {
		this.bak3 = bak3;
	}

	public void setBak4(String bak4) {
		this.bak4 = bak4;
	}

	public void setBak5(String bak5) {
		this.bak5 = bak5;
	}

	public void setBak9(String bak9) {
		this.bak9 = bak9;
	}

	public void setBelongbsc(String belongbsc) {
		this.belongbsc = belongbsc;
	}

	public void setBelongbts(String belongbts) {
		this.belongbts = belongbts;
	}

	public void setBelongs(String belongs) {
		this.belongs = belongs;
	}

	public void setBelongsuburb(String belongsuburb) {
		this.belongsuburb = belongsuburb;
	}

	public void setBtslevel(String btslevel) {
		this.btslevel = btslevel;
	}

	public void setCellulartype(String cellulartype) {
		this.cellulartype = cellulartype;
	}

	public void setCleartype(String cleartype) {
		this.cleartype = cleartype;
	}

	public void setCom_stat(String com_stat) {
		this.com_stat = com_stat;
	}

	public void setCov_area(String cov_area) {
		this.cov_area = cov_area;
	}

	public void setCov_load(String cov_load) {
		this.cov_load = cov_load;
	}

	public void setCreateType(String createType) {
		this.createType = createType;
	}

	public void setCustomer(String customer) {
		this.customer = customer;
	}

	public void setDate_bak2(String date_bak2) {
		this.date_bak2 = date_bak2;
	}

	public void setDate_bak3(String date_bak3) {
		this.date_bak3 = date_bak3;
	}

	public void setDutyBelong(String dutyBelong) {
		this.dutyBelong = dutyBelong;
	}

	public void setDutyLocation(String dutyLocation) {
		this.dutyLocation = dutyLocation;
	}

	public void setEngineeremail(String engineeremail) {
		this.engineeremail = engineeremail;
	}

	public void setEngineerid(String engineerid) {
		this.engineerid = engineerid;
	}

	public void setEngineername(String engineername) {
		this.engineername = engineername;
	}

	public void setEngineertel(String engineertel) {
		this.engineertel = engineertel;
	}

	public void setEquipLevel(String equipLevel) {
		this.equipLevel = equipLevel;
	}

	public void setEquipType(String equipType) {
		this.equipType = equipType;
	}

	public void setFailuredescription(String failuredescription) {
		this.failuredescription = failuredescription;
	}

	public void setFaultautorectime(String faultautorectime) {
		this.faultautorectime = faultautorectime;
	}

	public void setFaulttitle(String faulttitle) {
		this.faulttitle = faulttitle;
	}

	public void setFeedbackinfo(String feedbackinfo) {
		this.feedbackinfo = feedbackinfo;
	}

	public void setFindTime(String findTime) {
		this.findTime = findTime;
	}

	public void setGzriselevel(String gzriselevel) {
		this.gzriselevel = gzriselevel;
	}

	public void setHanprofessional(String hanprofessional) {
		this.hanprofessional = hanprofessional;
	}

	public void setId(String id) {
		this.id = id;
	}

	public void setInitiator(String initiator) {
		this.initiator = initiator;
	}

	public void setInitiatorDepartment(String initiatorDepartment) {
		this.initiatorDepartment = initiatorDepartment;
	}

	public void setInitiatorTel(String initiatorTel) {
		this.initiatorTel = initiatorTel;
	}

	public void setInmsalarmflag(String inmsalarmflag) {
		this.inmsalarmflag = inmsalarmflag;
	}

	public void setInserttime(String inserttime) {
		this.inserttime = inserttime;
	}

	public void setIsAdjustment(String isAdjustment) {
		this.isAdjustment = isAdjustment;
	}

	public void setIsAnyQuestion(String isAnyQuestion) {
		this.isAnyQuestion = isAnyQuestion;
	}

	public void setIsfind(String isfind) {
		this.isfind = isfind;
	}

	public void setIsWait(String isWait) {
		this.isWait = isWait;
	}

	public void setLatitude(String latitude) {
		this.latitude = latitude;
	}

	public void setLineMaintenanceName(String lineMaintenanceName) {
		this.lineMaintenanceName = lineMaintenanceName;
	}

	public void setLineMaintenanceNameid(String lineMaintenanceNameid) {
		this.lineMaintenanceNameid = lineMaintenanceNameid;
	}

	public void setLineMaintenanceNametp(String lineMaintenanceNametp) {
		this.lineMaintenanceNametp = lineMaintenanceNametp;
	}

	public void setLocation(String location) {
		this.location = location;
	}

	public void setLongitude(String longitude) {
		this.longitude = longitude;
	}

	public void setMaintainUnit(String maintainUnit) {
		this.maintainUnit = maintainUnit;
	}

	public void setNetworkLevel(String networkLevel) {
		this.networkLevel = networkLevel;
	}

	public void setNetworktype(String networktype) {
		this.networktype = networktype;
	}

	public void setNo(String no) {
		this.no = no;
	}

	public void setNodeCn(String nodeCn) {
		this.nodeCn = nodeCn;
	}

	public void setOperation_bak3(String operation_bak3) {
		this.operation_bak3 = operation_bak3;
	}

	public void setOppositeCreateDate(String oppositeCreateDate) {
		this.oppositeCreateDate = oppositeCreateDate;
	}

	public void setOrderId(String orderId) {
		this.orderId = orderId;
	}

	public void setOriAlarmId(String oriAlarmId) {
		this.oriAlarmId = oriAlarmId;
	}

	public void setOrientation(String orientation) {
		this.orientation = orientation;
	}

	public void setOrigin(String origin) {
		this.origin = origin;
	}

	public void setOriginTime(String originTime) {
		this.originTime = originTime;
	}

	public void setProcessLimit(String processLimit) {
		this.processLimit = processLimit;
	}

	public void setRejectContent(String rejectContent) {
		this.rejectContent = rejectContent;
	}

	public void setRejectreason(String rejectreason) {
		this.rejectreason = rejectreason;
	}

	public void setRejectreason2(String rejectreason2) {
		this.rejectreason2 = rejectreason2;
	}

	public void setRep_dis(String rep_dis) {
		this.rep_dis = rep_dis;
	}

	public void setResprocess(String resprocess) {
		this.resprocess = resprocess;
	}

	public void setResults(String results) {
		this.results = results;
	}

	public void setScopeextent(String scopeextent) {
		this.scopeextent = scopeextent;
	}

	public void setSendinstancename(String sendinstancename) {
		this.sendinstancename = sendinstancename;
	}

	public void setSerialNo(String serialNo) {
		this.serialNo = serialNo;
	}

	public void setSignInDate(String signInDate) {
		this.signInDate = signInDate;
	}

	public void setSla_assignetime(String sla_assignetime) {
		this.sla_assignetime = sla_assignetime;
	}

	public void setSla_closetime(String sla_closetime) {
		this.sla_closetime = sla_closetime;
	}

	public void setSla_createtime(String sla_createtime) {
		this.sla_createtime = sla_createtime;
	}

	public void setSla_resolutiontime(String sla_resolutiontime) {
		this.sla_resolutiontime = sla_resolutiontime;
	}

	public void setSpecialist(String specialist) {
		this.specialist = specialist;
	}

	public void setStatus(String status) {
		this.status = status;
	}

	public void setTeleAlarmreaanalysis(String teleAlarmreaanalysis) {
		this.teleAlarmreaanalysis = teleAlarmreaanalysis;
	}

	public void setTeleEquipLevel(String teleEquipLevel) {
		this.teleEquipLevel = teleEquipLevel;
	}

	public void setTeleSubAlarmreaanalysis(String teleSubAlarmreaanalysis) {
		this.teleSubAlarmreaanalysis = teleSubAlarmreaanalysis;
	}

	public void setTheme(String theme) {
		this.theme = theme;
	}

	public void setTransferDate(String transferDate) {
		this.transferDate = transferDate;
	}

	public void setTransferUserId(String transferUserId) {
		this.transferUserId = transferUserId;
	}

	public void setTransferUserName(String transferUserName) {
		this.transferUserName = transferUserName;
	}

	public void setTransferUserTel(String transferUserTel) {
		this.transferUserTel = transferUserTel;
	}

	public void setTroubAddress(String troubAddress) {
		this.troubAddress = troubAddress;
	}

	public void setTroubLevel(String troubLevel) {
		this.troubLevel = troubLevel;
	}

	public void setTroubRecoverCondition(String troubRecoverCondition) {
		this.troubRecoverCondition = troubRecoverCondition;
	}

	public void setTyneid(String tyneid) {
		this.tyneid = tyneid;
	}

	public void setUrgeContent(String urgeContent) {
		this.urgeContent = urgeContent;
	}

	public void setUrgeDate(String urgeDate) {
		this.urgeDate = urgeDate;
	}

	public void setUrgePerson(String urgePerson) {
		this.urgePerson = urgePerson;
	}

	public void setUrgePersonTel(String urgePersonTel) {
		this.urgePersonTel = urgePersonTel;
	}

	public void setValidateContent(String validateContent) {
		this.validateContent = validateContent;
	}

	public void setVendor(String vendor) {
		this.vendor = vendor;
	}

	public void setWaitAuditDate(String waitAuditDate) {
		this.waitAuditDate = waitAuditDate;
	}

	public void setWaitAuditPerson(String waitAuditPerson) {
		this.waitAuditPerson = waitAuditPerson;
	}

	public void setWaitAuditPersonTel(String waitAuditPersonTel) {
		this.waitAuditPersonTel = waitAuditPersonTel;
	}

	public void setWaitOpinion(String waitOpinion) {
		this.waitOpinion = waitOpinion;
	}

	public void setWaitreason(String waitreason) {
		this.waitreason = waitreason;
	}

	public void setWaitreason2(String waitreason2) {
		this.waitreason2 = waitreason2;
	}

	public void setWaitRes(String waitRes) {
		this.waitRes = waitRes;
	}

	public void setWglineMaintenanceName(String wglineMaintenanceName) {
		this.wglineMaintenanceName = wglineMaintenanceName;
	}

	public void setWglineMaintenanceNameid(String wglineMaintenanceNameid) {
		this.wglineMaintenanceNameid = wglineMaintenanceNameid;
	}

	public void setWglineMaintenanceNametp(String wglineMaintenanceNametp) {
		this.wglineMaintenanceNametp = wglineMaintenanceNametp;
	}

	public void setWoId(String woId) {
		this.woId = woId;
	}

	public void setWorkContent(String workContent) {
		this.workContent = workContent;
	}

}
