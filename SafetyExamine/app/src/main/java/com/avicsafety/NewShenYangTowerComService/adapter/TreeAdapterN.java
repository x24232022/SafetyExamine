...
import com.avicsafety.NewShenYangTowerComService.model.NodeN;
 ...
 * 
 */
public class TreeAdapterN extends BaseAdapter {
	...
	private List<NodeN> allsCache = new ArrayList<NodeN>();
	private List<NodeN> alls = new ArrayList<NodeN>();
	private TreeAdapterN oThis = this;
	...
	public TreeAdapterN(Context context, List<NodeN> nodes) {
		this.con = context;
	 ...
	 *            根节点
	 */
	public TreeAdapterN(Context context, NodeN rootNode) {
		this.con = context;
		this.lif = (LayoutInflater) con
				.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
		addNode(rootNode);
	...
	private void addNode(NodeN node) {
		alls.add(node);
		allsCache.add(node);
		if (node.isLeaf())
			return;
	...
	// 复选框联动
	private void checkNode(NodeN node, boolean isChecked) {
	 ...
	 * @return
	 */
	public List<NodeN> getSeletedNodes() {
